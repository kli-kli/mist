...
if [ "$1" = 'mist' ]; then
  ...
  exec ./bin/worker --name $2 --config configs/docker.conf  --run-options $4 --java-args "-Dmist.akka.cluster.seed-nodes.0=akka.tcp://mist@$IP:2551 -Dmist.akka.remote.netty.tcp.hostname=$MYIP -Dmist.akka.remote.netty.tcp.bind-hostname=$MYIP"
fi
